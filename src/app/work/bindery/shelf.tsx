...
import { bendSheet, createBook, type Book } from "./book";
import { makeBookMaps, makeContactShadowTexture, makeWoodTexture } from "./textures";
 ...
 *
 * Camera distance is derived rather than hardcoded because the thing being
 * framed changes size: volumes differ in height, a book roughly doubles its
 * width when the cover swings open, and the viewport aspect decides whether
 * width or height is the binding constraint. A fixed radius that suits a
 * closed book on a wide monitor puts the camera inside the front board of an
 * open one on a phone.
 */
function fitDistance(width: number, height: number, aspect: number) {
  const v = (V_FOV * Math.PI) / 180;
...
function shelfDistance(aspect: number) {
  ...
  return fitDistance(2.5, 2.4, aspect) * 1.1;
}
function detailDistance(size: { width: number; height: number }, aspect: number) {
  return fitDistance(size.width * 1.45, size.height * 1.28, aspect) * 1.06;
}
...
export function Shelf() {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      for (let i = 0; i < world.books.length; i += 1) {
        ...
        const targetRotation = Math.PI / 2 - (selected ? world.turn : 0) * (Math.PI / 2 + 0.42);
  ...
  useEffect(() => {
    ...
    const tint = cloth.clone().multiplyScalar(0.32);
    tint.offsetHSL(0, -0.06, 0.02);
    world.backgroundTarget = tint;
    world.dirty = true;
  }, [volume]);
